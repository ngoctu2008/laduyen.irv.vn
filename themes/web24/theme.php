...
function nv_site_theme($contents, $full = true)
{
    ...
    if ($global_config['current_theme_type'] == 'r') {
        $html_links[] = array(
            'rel' => 'StyleSheet',
            'href' => NV_BASE_SITEURL . 'themes/' . $global_config['module_theme'] . '/css/bootstrap_new2.min.css'
        );
        // $html_links[] = array(
        //     'rel' => 'StyleSheet',
        ...
        $html_links[] = array(
            'rel' => 'StyleSheet',
            'href' => NV_BASE_SITEURL . 'style.php/style.scss'
        );
        $html_links[] = array(
            'rel' => 'StyleSheet',
            'href' => NV_BASE_SITEURL . 'themes/' . $global_config['module_theme'] . '/css/webmenu.css'
        );
        $html_links[] = array(
            'rel' => 'StyleSheet',
    ...
    if (preg_match("/<h2[^\>]*\>/i", $contents)) {
        $xtpl->parse('main.site_description_span');
    } else {
        $xtpl->parse('main.site_description_h2');
    }
    ...
    if ($full) {
        ...
        if (!$home) {
            ...
            if (!empty($array_mod_title_copy)) {
                $pos = 2;
				$num_last = count($array_mod_title_copy);
				$array_mod_title_copy[$num_last - 1]['link'] = '';
                foreach ($array_mod_title_copy as $arr_cat_title_i) {
                    $xtpl->assign('BREADCRUMBS_POS', $pos);
                    $xtpl->assign('BREADCRUMBS', $arr_cat_title_i);
					$pos++;
					if(!empty($arr_cat_title_i['link'])){
						$xtpl->parse('main.breadcrumbs.loop.href');
					}
                    $xtpl->parse('main.breadcrumbs.loop');
                }
            }
            $xtpl->assign('TITLE', $arr_cat_title_i['title']);
