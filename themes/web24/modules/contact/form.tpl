...
<div class="nv-fullbg row">
    <form method="post" class="form_contact row" action="{ACTION_FILE}" onsubmit="return nv_validForm(this);" novalidate>
		...
		<!-- END: ccats -->
		<div class="col-xs-24 contact-title">{LANG.send_info}</div>
		<div class="col-xs-24 contact-note"><span class="red">*</span> {LANG.is_required}</div>
		<div class="form-group hidden">
			<div class="">
				<input type="text" maxlength="255" class="form-control form_input required" value="Gửi liên hệ" name="ftitle" placeholder="{LANG.title}" data-pattern="/^(.){3,}$/" onkeypress="nv_validErrorHidden(this);" data-mess="{LANG.error_title}" />
			</div>
		</div>
		<!-- BEGIN: iguest -->
		...
		<div class="form-group col-xs-24 col-sm-8">
			<div class="">
                <input type="number" maxlength="60" value="{CONTENT.fphone}" name="fphone" class="form-control form_input required" placeholder="{LANG.phone}" onkeypress="nv_validErrorHidden(this);" data-mess="{LANG.error_content}"/>
            </div>
        </div>
        <div class="form-group hidden">
			<div class="">
		...
		<div class="form-group col-xs-24">
            <div>
    			<textarea cols="8" rows="6" name="fcon" class="form-control form_input " maxlength="1000" placeholder="{LANG.content}"></textarea>
            </div>
		</div>
        <!-- BEGIN: sendcopy -->
		...
		<div class="text-center form-group col-xs-24">
			...
			<input type="submit" value="{LANG.sendcontact}" name="btsend" class="btn-send_contact btn_cus -bg_primary -white -brd_primary" />
		</div>
	</form>
