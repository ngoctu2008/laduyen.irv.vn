...
if (!nv_function_exists('nv_post')) {
    ...
    function nv_post($block_config)
    {
        ...
        $db->sqlreset()->select('id, title, alias, description, image')->from(NV_PREFIXLANG . '_' . $site_mods[$module]['module_data'])->where('id = '.$block_config['id_post']);
        ...
        if (!empty($list)) {
            ...
            foreach ($list as $l) {
                $l['title_clean60'] = nv_clean60($l['title'], $block_config['title_length'], false);
                $l['description_clean60'] = nv_clean60(strip_tags($l['description'],'<br>'), $block_config['content_length'], false);
                ...
                $xtpl->assign('BL_CONFIG', $block_config);
                $xtpl->assign('ROW', $l);
