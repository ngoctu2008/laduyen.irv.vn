...
if (!nv_function_exists('nv_block_freecontent_camket')) {
    /**
     * nv_block_config_freecontent_camket()
     *
     * @param mixed $module
     ...
     * @return
     */
    function nv_block_config_freecontent_camket($module, $data_block, $lang_block)
    {
    ...
    /**
     * nv_block_config_freecontent_camket_submit()
     *
     * @param mixed $module
     * @param mixed $lang_block
     * @return
     */
    function nv_block_config_freecontent_camket_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
    ...
    /**
     * nv_block_freecontent_camket()
     *
     * @return
     */
    function nv_block_freecontent_camket($block_config)
    {
        ...
        if (!empty($list)) {
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $site_mods[$module]['module_file'] . '/block.free_content_camket.tpl')) {
                $block_theme = $global_config['module_theme'];
            } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/' . $site_mods[$module]['module_file'] . '/block.free_content_camket.tpl')) {
                $block_theme = $global_config['site_theme'];
            } else {
                $block_theme = 'default';
            }

            $xtpl = new XTemplate('block.free_content_camket.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $site_mods[$module]['module_file']);
...
if (defined('NV_SYSTEM')) {
    $content = nv_block_freecontent_camket($block_config);
}
