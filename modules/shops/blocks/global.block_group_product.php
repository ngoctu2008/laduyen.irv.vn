...
if (!nv_function_exists('nv_group_product')) {
     ...
     *
     */
    function nv_block_config_group_blocks($module, $data_block, $lang_block)
    {
        ...
        foreach ($list as $l) {
			...
			$l[NV_LANG_DATA . '_alias'] = NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&amp;' . NV_NAME_VARIABLE . '=' . $l[NV_LANG_DATA . '_alias'];
            $html .= "<option value=\"" . $l['bid'] . "\" " . $sel . " data-alias=\"" .$l[NV_LANG_DATA . '_alias']. "\">" . $l[NV_LANG_DATA . '_title'] . "</option>\n";
        }
        $html .= "	</select></div>\n";
        ...
        $html .= '		$("input[name=link]").val($("select[name=config_blockid] option:selected").data("alias"));';
        $html .= '	});';
        $html .= '</script>';
        $html .= "</div>";
     ...
     *
     */
    function nv_block_config_group_blocks_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
     ...
     *
     */
    function nv_group_product($block_config)
    {
        global $nv_Cache, $site_mods, $global_config, $lang_module, $module_config, $module_config, $module_name, $module_info, $global_array_shops_cat, $db_config, $my_head, $db, $pro_config, $money_config, $array_wishlist_id;
        ...
        if ($module != $module_name) {
            ...
            // Lay ty gia ngoai te
            $sql = 'SELECT code, currency, exchange, symbol, round, number_format FROM ' . $db_config['prefix'] . '_' . $mod_data . '_money_' . NV_LANG_DATA;
        ...
        $xtpl = new XTemplate('block.group_product.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $mod_file);
        $xtpl->assign('LANG', $lang_module);
        $xtpl->assign('WIDTH', $pro_config['homewidth']);

        $db->sqlreset()
            ->select('t1.id, t1.listcatid, t1.' . NV_LANG_DATA . '_title AS title, t1.' . NV_LANG_DATA . '_alias AS alias, t1.addtime, t1.homeimgfile, t1.homeimgthumb, t1.product_price, t1.money_unit, t1.discount_id, t1.showprice, t1.ratingdetail, t1.product_number, t1.allowed_rating')
            ->from($db_config['prefix'] . '_' . $mod_data . '_rows t1')
            ->join('INNER JOIN ' . $db_config['prefix'] . '_' . $mod_data . '_block t2 ON t1.id = t2.id')
            ->where('t2.bid= ' . $block_config['blockid'] . ' AND t1.status =1')
            ->order('t2.weight ASC, t1.publtime DESC')
        ...
        foreach ($list as $row) {
			...
			if($pro_config['review_active'] && $row['allowed_rating']){
				$countAll = $average = $score = 0;
				$array_rating = $db->query('SELECT rating FROM ' . $db_config['prefix'] . '_' . $mod_data . '_review WHERE product_id = '. $row['id'])->fetchAll();
				...
				for ($i=1; $i <= $average; $i++){
					$xtpl->parse('main.loop.rating.star');
				}
				$xtpl->assign('count_rate', $countAll);
				$xtpl->parse('main.loop.rating');
			}
...
if (defined('NV_SYSTEM')) {
    $content = nv_group_product($block_config);
}
