...
if (!nv_function_exists('nv_block_freecontent_quytrinh')) {
    /**
     * nv_block_config_freecontent_quytrinh()
     *
     * @param mixed $module
     ...
     * @return
     */
    function nv_block_config_freecontent_quytrinh($module, $data_block, $lang_block)
    {
    ...
    /**
     * nv_block_config_freecontent_quytrinh_submit()
     *
     * @param mixed $module
     * @param mixed $lang_block
     * @return
     */
    function nv_block_config_freecontent_quytrinh_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
    ...
    /**
     * nv_block_freecontent_quytrinh()
     *
     * @return
     */
    function nv_block_freecontent_quytrinh($block_config)
    {
        ...
        $sql = 'SELECT id, title, description, image, icon, link, target FROM ' . NV_PREFIXLANG . '_' . $site_mods[$module]['module_data'] . '_rows WHERE status = 1 AND bid = ' . $block_config['blockid'];
        $list = $nv_Cache->db($sql, 'id', $module);

        if (!empty($list)) {
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $site_mods[$module]['module_file'] . '/block.free_content_quytrinh.tpl')) {
                $block_theme = $global_config['module_theme'];
            } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/' . $site_mods[$module]['module_file'] . '/block.free_content_quytrinh.tpl')) {
                $block_theme = $global_config['site_theme'];
            } else {
                $block_theme = 'default';
            }

            $xtpl = new XTemplate('block.free_content_quytrinh.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $site_mods[$module]['module_file']);
...
if (defined('NV_SYSTEM')) {
    $content = nv_block_freecontent_quytrinh($block_config);
}
