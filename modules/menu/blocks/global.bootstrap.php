...
if (! nv_function_exists('nv_menu_bootstrap')) {
        ...
        if (! empty($array_menu[$id])) {
            foreach ($array_menu[$id] as $sid => $smenu) {
                ...
                if (isset($array_menu[$sid])) {
                    $submenu = nv_get_bootstrap_subsubmenu($sid, $array_menu, $submenu_active, $block_theme);
                    $xtpl->assign('SUB', $submenu);
                    $xtpl->parse($type_submenu.'.loop.item');
                }
                $xtpl->assign('SUBMENU', $smenu);
                if (! empty($submenu)) {
                    $xtpl->parse($type_submenu.'.loop.submenu');
                }
                if (! empty($smenu['icon'])) {
                    $xtpl->parse($type_submenu.'.loop.icon');
                }
                $xtpl->parse($type_submenu.'.loop');
            }
            $xtpl->parse($type_submenu);
        }
        return $xtpl->text($type_submenu);
    }
     ...
     * @return string
     */
    function nv_get_bootstrap_subsubmenu($id, $array_menu, &$submenu_active, $block_theme)
    {
		// Code from bamboospa.com.vn
        $type_showsub = 0;
        if($type_showsub == 0){
            $type_submenu = 'subsubmenu_car';
        }
        else{
            $type_submenu = 'subsubmenu';
        }
        $xtpl = new XTemplate('global.bootstrap.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/menu');
