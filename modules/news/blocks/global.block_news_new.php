...
if (!nv_function_exists('nv_news_block_news_new')) {
    /**
     * nv_block_config_news_new_blocks()
     *
     * @param mixed $module
     ...
     * @return
     */
    function nv_block_config_news_new_blocks($module, $data_block, $lang_block)
    {
    ...
    /**
     * nv_block_config_news_new_blocks_submit()
     *
     * @param mixed $module
     * @param mixed $lang_block
     * @return
     */
    function nv_block_config_news_new_blocks_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
    ...
    /**
     * nv_news_block_news_new()
     *
     * @param mixed $block_config
     * @param mixed $mod_data
     * @return
     */
    function nv_news_block_news_new($block_config, $mod_data)
    {
        ...
        $db_slave->sqlreset()
            ->select('id, catid, publtime, title, alias, homeimgthumb, homeimgfile, hometext, external_link, publtime')
        ...
        if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $mod_file . '/block_news_new.tpl')) {
            $block_theme = $global_config['module_theme'];
        } else {
            $block_theme = 'default';
        }

        $xtpl = new XTemplate('block_news_new.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $mod_file);
        ...
        foreach ($array_block_news as $array_news) {
            ...
            $array_news['hometext_clean'] = nv_clean60($array_news['hometext_clean'], 160, true);

            if ($array_news['external_link']) {
                $array_news['target_blank'] = 'target="_blank"';
            }
            $dmy = nv_date('l, d F, Y', $array_news['publtime']);
            $xtpl->assign('dmy', $dmy);
            $xtpl->assign('blocknews', $array_news);

            if (!empty($array_news['imgurl'])) {
                $xtpl->parse('main.other.imgblock');
            }
...
if (defined('NV_SYSTEM')) {
    ...
    if (isset($site_mods[$module])) {
        ...
        $content = nv_news_block_news_new($block_config, $mod_data);
    }
}
